...
**Scope note for Phase C.3:** nothing in the current codebase inserts a `workflow_instances` row — `workflow-research-run`/`workflow-governance-run` both operate on an existing `instanceId` passed in; only `startInstance()` in `workflowEngine.ts` creates one, and nothing calls it yet. Phase C.3's trigger (off `agent_runs` INSERT) will need to call `startInstance()` itself, not just invoke the existing three functions. Also unresolved: `resolveCaller` (used by all three functions) requires a real authenticated organisation-member user's JWT — there is no service-role fast path today, unlike `embedding-pipeline-run`'s `assertPlatformCaller`. An automated background trigger has no real user session to present, so Phase C.3 needs its own answer to this before it can call these functions itself, not just wire a webhook.

Per §7, stopping here for review — Phase C.3 (wiring the shadow invocation) is not started.
