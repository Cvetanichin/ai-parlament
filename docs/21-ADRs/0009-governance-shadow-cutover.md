...
**Phase C.2 complete.** `workflow-research-run`, `workflow-governance-run`, `workflow-gate-decide` deployed to production — first time any new Parliament Core / Quorum Engine *application logic* (not just schema) has run in production. Verified before and after: `me-agent`, `compliance-agent`, `reporting-agent`, and every other pre-existing production function are byte-identical (same version numbers, same content hashes) to before this deploy — zero impact on the live app, since nothing calls the three new functions yet.

Each function now reads `GOVERNANCE_MODE` at invocation time (`Deno.env.get("GOVERNANCE_MODE") ?? "shadow"`) and reports it in its JSON response. Defaults to `"shadow"` when unset so no secret has to be configured for this phase to be meaningful; `"enforced"` is defined but not yet activated — nothing branches on it until Phase C.6 re-platforms the live ministries.

**Front-loaded, at the Product Owner's direction:** §4 Phase C.3's open question (how to tag a shadow verdict as distinguishable from a future enforced-mode verdict) is resolved now rather than deferred. Added `workflow_instances.governance_mode` (nullable text, `check (... in ('shadow','enforced'))`) via migration `15_governance_mode_column.sql`, applied to staging then production. Nullable and undefaulted: only instances created by the automated shadow/enforced invocation path (Phase C.3 onward) get a value; manually/directly created instances (e.g. House of Parliament testing) stay `NULL` — a deliberate third bucket.

**Two of the three Phase C.3 blockers below are now resolved; a third, more fundamental one was found while investigating them.**

1. **RESOLVED — auth for an automated caller.** `resolveCaller` (`_shared/auth.ts`, used by all three functions) required a real authenticated organisation-member user's JWT, with no path for a system-initiated call. Added a `service_role` fast path (mirrors `embedding-pipeline-run`'s `assertPlatformCaller`, ADR-0010 §6): a service-role caller resolves `organisationId` from the target project's row and gets a synthetic `{userId: "system", role: "system"}` context. `requireGateRole` (used only by `workflow-gate-decide`) still correctly rejects `role: "system"` — this is deliberate, not an oversight: it's what keeps an automated trigger permanently unable to reach a human gate decision, per EAS §9's Liability NFR. Deployed to staging then production, verified zero new security lints on both.

2. **Not yet solved — nothing creates a `workflow_instances` row.** `workflow-research-run`/`workflow-governance-run` both operate on an existing `instanceId` passed in; only `startInstance()` in `workflowEngine.ts` creates one, and nothing calls it yet. Phase C.3's trigger will need to call it itself, not just invoke the existing three functions. Straightforward to build once the item below is resolved — the two are linked, because what `startInstance()` should be called *with* depends on it.

3. **NEW, more fundamental — the Tripartite Veto Engine has no meaningful input for real ministry output.** Checked live production `agent_runs` data before building the trigger: `me-agent`/`compliance-agent`/`reporting-agent` write `input_data` shaped like `{period_start, period_end}` and `output_data` shaped like `{title: "M&E Brief — HERA VOL 2 (...)"}`" — no narrative "draft" text field at all. (Also: 5 of the last 6 real runs have `status: "error"`, `output_data: null`.) `runVeto()` requires a `draft: string`, `brief: string`, and `VetoConstraints {characterLimit, requiredKeywords}` — concepts built for Grant Studio's proposal-writing workflow, not this SaaS product's M&E/compliance/reporting outputs. Its semantic-check prompt literally says "Score whether this Annex A draft... addresses the project brief," which has no sensible referent for a compliance or reporting brief. There is no clean, non-invented mapping from what these three live ministries actually produce onto what the veto engine expects to score. **This blocks Phase C.3's trigger from being built as literally specified** ("submits that run's `input_data`/`output_data` to `workflow-governance-run`") until decided: does Phase C.3 need a different, purpose-built shadow-scoring path (e.g. running only `runVeto` against real content, with a genuinely generic semantic prompt not anchored to "Annex A"/"project brief"), or does something else change first? Not invented here — needs your call.

Per §7, stopping here for review — Phase C.3 (wiring the shadow invocation) is not started, blocked on item 3 above.
