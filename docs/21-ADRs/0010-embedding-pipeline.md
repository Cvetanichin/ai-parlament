...
- §10 backfill against the 75 `regulatory_clauses` rows is blocked on `OPENAI_API_KEY` billing (the OpenAI account behind the Vault secret currently returns `insufficient_quota`) — a provider-account precondition, not an implementation gap. The function itself is deployed, authorised, and verified to fail closed and audit correctly on error.
